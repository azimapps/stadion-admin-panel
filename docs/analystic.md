...
**GET** `/payment-methods?date_from=2026-03-01&date_to=2026-03-31`

Returns total revenue and transaction count broken down by payment method (Payme, Click, Cash) for a flexible date range. Useful for seeing exactly how much came in via each payment provider.
...
| Param | Type | Required | Description |
|-------|------|----------|-------------|
| date_from | date (YYYY-MM-DD) | Yes | Start of range (inclusive) |
| date_to | date (YYYY-MM-DD) | Yes | End of range (inclusive) |
| stadium_id | int | No | Filter to a specific stadium |
...
**summary** — one entry per method that had at least one transaction:

| Field | Description |
|-------|-------------|
| `<method>.total` | Total amount received via this method (UZS) |
| `<method>.count` | Number of transactions via this method |
| total | Grand total across all methods (UZS) |

Methods returned: `payme`, `click`, `cash`. Only methods with transactions in the range appear.

### Notes

- `payme_merchant` and `click_merchant` (auto-recorded by webhooks) are merged into `payme` and `click`
- When `stadium_id` is provided, only stadium booking payments are counted — tournament payments are excluded since tournaments are not tied to a single stadium
- Amounts are in UZS
