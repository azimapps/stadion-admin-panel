...
import { ImagePlus, Loader2, GripVertical, Star, X, Trash2 } from "lucide-react"
...
export function ImageUploader({ images, onChange }: ImageUploaderProps) {
    ...
    const handleFiles = async (files: FileList | null) => {
        ...
        try {
            ...
            for (const file of list) {
                const res = await marketplaceService.uploadImage(file)
            ...
            toast.success(
                uploaded.length === 1
                    ? "Rasm yuklandi"
                    : `${uploaded.length} ta rasm yuklandi`
            )
        } catch (err) {
            ...
            >
                <div className="relative flex flex-col items-center gap-3 py-10 px-6 text-center">
                    ...
                    <div>
                        ...
                        <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/70 mt-1">
                            JPEG · PNG · WEBP · Maks 1 MB
                        </div>
                    </div>
                </div>
            </button>
